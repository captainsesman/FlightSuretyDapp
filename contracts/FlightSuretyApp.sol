...
contract FlightSuretyApp {
    ...
    FlightSuretyData flightSuretyData; //Instantiating FlightSuretyData to be Used
    ...
    modifier requireAirlineCanVote() {
        require(
            flightSuretyData.canVoteStatus(msg.sender),
            "Caller is Can Not Vote"
        );
        _;
    }
    ...
    function isAirline(address airlineAccount) public view returns (bool) {
        return (flightSuretyData.isAirline(airlineAccount));
    }
    ...
    function approveAirlineRegistration(address _airline)
        external
        requireIsOperational
        requireAirlineCanVote
        returns (bool)
    {
        //Airline Should Vote if Not Voted
        flightSuretyData.vote(_airline, msg.sender);
        uint256 voteCount = flightSuretyData.getAirlineVoteCount(_airline);
        uint256 canVoteAirlineCount = flightSuretyData
            .getTotalCanVoteAirlines();

        if (voteCount >= canVoteAirlineCount.div(2)) {
            flightSuretyData.setisApproved(_airline);
            flightSuretyData.resetAirlineVotes(_airline);
            return true;
        } else {
            return false;
        }
